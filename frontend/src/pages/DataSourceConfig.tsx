...
const DataSourceConfig = () => {
    ...
    useEffect(() => {
        if (users.length > 0) {
            setLocalUsers(prev => {
                // Only initialize if empty to avoid overwriting user changes
                // Note: This assumes initial load. If strict sync is needed, logic should be different.
                return prev.length === 0 ? users : prev;
            });
        }
    ...
    useEffect(() => {
        if (sites.length > 0) {
            setLocalSites(prev => {
                return prev.length === 0 ? sites : prev;
            });
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [sites]);
